...
var $n_iflags = 0;
export var flag_byname = Object.create(null);
export var flag_bynum = [];
var flag_fields = [];
var $iflag_words;
function if_($name, $def) {
    var $num = $n_iflags++;
    var $v = [$num, $name, $def];
...
function if_align($name) {
    if (flag_fields.length > 0) {
        flag_fields[flag_fields.length - 1][2] = $n_iflags - 1;
    }
    $n_iflags = ($n_iflags + 31) & ~31;
    if ($name) {
        flag_fields.push([$name, $n_iflags, undefined]);
    }
    return 1;
}
...
export var insns_flag_hash = Object.create(null);
export var insns_flag_values = [];
export var insns_flag_lists = [];

function getFlagIndex(args) {
    if (args[0] === "ignore") return;
    var prekey = args.sort();
    var $key = prekey.join(',');
    var $flag_index = insns_flag_hash[$key];

    if ($flag_index === undefined) {
        var newkey = new Array($iflag_words).fill(0);

        for (var $i of prekey) {
            var $flag = flag_byname[$i];
            if ($flag === undefined) throw `未找到已定义的标识${$i}(${$key})`;
            newkey[$flag[0] >> 5] |= (1 << ($flag[0] & 31));
        }
        insns_flag_values.push(newkey);
        insns_flag_lists.push($key);
        insns_flag_hash[$key] = $flag_index = insns_flag_values.length - 1;
    }

    return $flag_index;
}

export default getFlagIndex;